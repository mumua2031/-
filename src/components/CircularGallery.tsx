...
function autoBind(instance: object) {
  ...
  Object.getOwnPropertyNames(proto).forEach((key) => {
    ...
    if (key !== 'constructor' && typeof value === 'function') {
      (instance as Record<string, unknown>)[key] = (value as Function).bind(instance);
    }
  });
}
  ...
  try {
    ...
    if (document.fonts?.load) {
      try {
        ...
        // Render with best available font.
      }
    }
    return resolved;
  } catch (error) {
...
function createTextTexture(gl: any, text: string, font = DEFAULT_FONT, color = '#ffffff') {
...
class Title {
  ...
  constructor({ gl, plane, text, textColor = '#ffffff', font = DEFAULT_FONT }: any) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
    this.text = text;
...
class Media {
  ...
  update(scroll: { current: number; last: number }, direction: 'left' | 'right') {
    ...
    if (this.bend === 0) {
      ...
      const radius = (span * span + bendAbs * bendAbs) / (2 * bendAbs);
      const effectiveX = Math.min(Math.abs(x), span);
...
class App {
  ...
  createMedias(items: GalleryItem[] | undefined, bend = 1, textColor = '#ffffff', borderRadius = 0, font = DEFAULT_FONT) {
